...
class Event:
    ...
    def post_events_search(self, request: operations.PostEventsSearchRequest) -> operations.PostEventsSearchResponse:
        r"""Search events
        ...
        res = operations.PostEventsSearchResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[operations.PostEventsSearch200ApplicationJSON])
                res.post_events_search_200_application_json_object = out

        return res

    
    def post_ingest(self, request: operations.PostIngestRequest) -> operations.PostIngestResponse:
        r"""Ingest events
        ...
        res = operations.PostIngestResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[operations.PostIngest200ApplicationJSON])
                res.post_ingest_200_application_json_object = out
        elif r.status_code == 400:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[operations.PostIngest400ApplicationJSON])
                res.post_ingest_400_application_json_object = out

        return res

    
    def put_events_deprecate_event_id(self, request: operations.PutEventsDeprecateEventIDRequest) -> operations.PutEventsDeprecateEventIDResponse:
        ...
        * The `customer_id` or the `external_customer_id` of the original event ingestion request must identify a Customer resource within Orb, even if this event was ingested during the initial integration period. We do not allow deprecating events for customers not in the Orb system.
        ...
        url = utils.generate_url(base_url, "/events/{event_id}/deprecate", request.path_params)
        ...
        res = operations.PutEventsDeprecateEventIDResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[operations.PutEventsDeprecateEventID200ApplicationJSON])
                res.put_events_deprecate_event_id_200_application_json_object = out
        elif r.status_code == 400:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[operations.PutEventsDeprecateEventID400ApplicationJSON])
                res.put_events_deprecate_event_id_400_application_json_object = out

        return res

    
    def put_events_event_id(self, request: operations.PutEventsEventIDRequest) -> operations.PutEventsEventIDResponse:
        ...
        * The event's `timestamp` must fall within the customer's current subscription's billing period, or within the grace period of the customer's current subscription's previous billing period.
        ...
        url = utils.generate_url(base_url, "/events/{event_id}", request.path_params)
        ...
        res = operations.PutEventsEventIDResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[operations.PutEventsEventID200ApplicationJSON])
                res.put_events_event_id_200_application_json_object = out
        elif r.status_code == 400:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[operations.PutEventsEventID400ApplicationJSON])
